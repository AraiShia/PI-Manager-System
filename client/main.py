...
class MainWindow(QMainWindow):
    ...
    def switch_tab(self, key):
        ...
        if key == "pi":
            ...
            refresh_map = {
                ...
                "order_summary": self._update_order_summary_view,
            }
    ...
    def _on_order_detail_double_click(self, row, column):
        ...
        # ===== v1.1: 临时产品权限控制 =====
        if items and row < len(items):
            item = items[row]
            # 2026-06-12 修复：转成 bool 防止后端返回 0/1 整数
            item_is_temporary = bool(item.get('is_temporary', False))
            # 临时产品判断：product_id 缺失/为0，或 is_temporary 标记为 True
    ...
    def _handle_temporary_product_edit(self, row, column, item, order):
        ...
        dialog.product_confirmed.connect(
            # 2026-06-16 修复：信号改为 (bool, dict)，updated_item 现在是 API 返回的完整字典
            lambda success, updated: self._on_temp_product_confirmed(success, updated, order)
        )

        dialog.exec()

    def _on_temp_product_confirmed(self, success, updated_item, order):
        """临时产品转正成功后回调 - 同步价格/备注到 PI item 并刷新数据"""
        # 2026-06-16 修复：success 参数新签名；updated_item 现在是 API 返回的完整字典（含 customer_remark）
        if not success or not updated_item:
            print("[INFO] 转正未成功，跳过同步")
            return

        # 2026-06-23 修复：先前在下面"清除缓存"，但下面又要从缓存读 items 来定位 PI item，
        # 顺序冲突导致 get_items_by_order_id() 永远返回 []，同步循环根本不执行，
        # 后果：PI item 的 is_temporary 永远没被改成 False，下次双击仍弹转正 Dialog。
        # 修复方案：直接从 API 拉一次最新 PI 详情，绕开缓存状态。
        pi_id_for_sync = order.get('pi_id') or order.get('id')
        fresh_items = []
        if pi_id_for_sync:
            try:
                fresh_detail = self.api_client.get_pi_detail(pi_id_for_sync) or {}
                fresh_items = fresh_detail.get('items', []) or []
            except Exception as e:
                print(f"[WARN] 转正后拉取最新 PI 详情失败: {e}")

        # 兼容：updated_item 可能是 dict 或 int（向后兼容旧信号）
        if isinstance(updated_item, int):
            product_id = updated_item
            updated_item = self.api_client.get_product_detail(product_id) or {}
        else:
            product_id = updated_item.get('id') or updated_item.get('product_id')
        if not product_id:
            return

        # 2026-06-09 修复：转正只更新了产品表(PrdProduct)，未同步到 PI 订单项(PiProformaInvoiceItem)
        # 导致：表格报价/合计金额仍为0，编辑对话框备注仍显示旧值
        if order and pi_id_for_sync:
            # 2026-06-23 修复：order 来自 pi_list（无 items 字段），必须从 OrderService 的
            # items_cache 读取，否则同步循环不会执行 → is_temporary=False 无法下发。
            order_id = order.get('id')
            items = fresh_items  # ← 直接用 API 拉到的最新 items
            if not items and order_id and hasattr(self, '_order_summary_tab'):
                # API 失败时回退到缓存（虽然此时缓存可能为空，但保留兜底）
                items = self._order_summary_tab.get_service().get_items_by_order_id(order_id) or []
            if not items:
                items = order.get('items', []) or []

            # 找到对应 product_id 的 PI item
            for item in items:
                if item.get('product_id') == product_id or str(item.get('product_id')) == str(product_id):
                    pi_item_id = item.get('id')
                    if pi_item_id:
                        # 用转正后的产品数据同步更新 PI item
                        sync_data = {}
                        # 2026-06-16 修复：转正成功后强制把 PI item 标记为正式（is_temporary=False）
                        # 这是后端 get_pi_invoice_detail 的最高优先级来源
                        sync_data['is_temporary'] = False
                        # Col 5: 产品名称
                        detail_desc = updated_item.get('detail_desc') or updated_item.get('product_name')
                        if detail_desc:
                            sync_data['detail_desc'] = detail_desc
                        # Col 7: 客户型号
                        customer_model = updated_item.get('customer_model')
                        if customer_model:
                            sync_data['customer_model'] = customer_model
                        # Col 10: 报价（优先 price_rmb，RMB 报价）
                        price_rmb = updated_item.get('price_rmb')
                        price_usd = updated_item.get('price_usd')
                        if price_rmb and float(price_rmb) > 0:
                            sync_data['unit_price'] = float(price_rmb)
                        elif price_usd and float(price_usd) > 0:
                            sync_data['unit_price'] = float(price_usd)
                        # Col 4: 客户备注（关键修复：customer_remark 现在能正确传到 PI item）
                        remark = updated_item.get('customer_remark') or updated_item.get('remark')
                        if remark:
                            sync_data['remark'] = remark
                        # Col 39: 品牌
                        brand = updated_item.get('brand')
                        if brand:
                            sync_data['brand'] = brand
                        # Col 3: OE号
                        oe_number = updated_item.get('oe_number')
                        if oe_number:
                            sync_data['oe_number'] = oe_number
                        if sync_data:
                            try:
                                result = self.api_client.update_pi_item(pi_item_id, sync_data)
                                if result and result.get('success'):
                                    print(f"[INFO] 转正后同步 PI item {pi_item_id}: {list(sync_data.keys())}")
                                else:
                                    print(f"[WARN] 转正后同步 PI item 失败: {result}")
                            except Exception as e:
                                print(f"[ERROR] 转正后同步 PI item 异常: {e}")
                    break

        # 刷新订单数据
        self._update_order_summary_view()

        # 2026-06-23 修复：临时产品转正后同步刷新产品管理列表
        # 避免产品管理中仍显示旧的临时产品名称
        try:
            self.load_products(use_cache=False)
            print("[INFO] 转正后已刷新产品管理列表")
        except Exception as e:
            print(f"[WARN] 转正后刷新产品管理列表失败: {e}")

    def _show_temporary_readonly_dialog(self, column):
        """显示临时产品只读提示对话框"""
        headers = [self.order_detail_table.horizontalHeaderItem(i).text()
                   for i in range(self.order_detail_table.columnCount())]
        column_name = headers[column] if column < len(headers) else f"列{column}"
        
        QMessageBox.information(
            self,
            "⚠️ 临时产品限制",
            f"【{column_name}】字段不可编辑\n\n"
            "原因: 该产品为临时产品，此字段需要转为正式产品后才能编辑。\n\n"
            "解决方案:\n"
            "1. 右键点击该行 → 选择「转正为正式产品」\n"
            "2. 在向导中补全供应商、包装等信息\n"
            "3. 转正完成后即可编辑所有字段",
