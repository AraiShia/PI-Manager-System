...
class ShShipmentItem(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    shipment_id = Column(Integer, ForeignKey("sh_shipment.id"), nullable=False)
    stage_id = Column(Integer, ForeignKey("sh_shipment_stage.id"), nullable=True)  # 关联到具体出货阶段
    pi_item_id = Column(Integer)                   # 关联PI明细ID
    product_id = Column(Integer, ForeignKey("prd_product.id"), nullable=False)
    quantity = Column(DECIMAL(15, 4), nullable=False)
    unit_price = Column(DECIMAL(15, 4))           # 单价
    total_price = Column(DECIMAL(15, 4))           # 总价
    carton_no = Column(String(100))               # 箱号
    net_weight = Column(DECIMAL(12, 4))          # 净重
    gross_weight = Column(DECIMAL(12, 4))         # 毛重
    dimension = Column(String(200))               # 尺寸
